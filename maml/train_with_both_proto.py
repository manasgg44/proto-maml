...
def find_tsne(emdeddings, classes, path):
    ...
    for cls in unique_classes :
        ...
        plt.scatter(embedddings_2d[idx, 0], embeddings_2d[idx, 1], label=cls)
    plt.title(save_path)
    plt.legend()
    plt.savefig('figure')
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                train_embeddings = model.forward_enc(train_input,params=params)
                if batch_idx == 99 and task_idx ==1 :
                    find_tsne(train_embeddings_original, train_target, 'train_embeddings.png')
