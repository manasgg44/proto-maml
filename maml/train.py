...
def compute_contrastive_loss(embeddings, labels, margin=1.0):
    ...
    pairs, pair_labels = create_pairs(embeddings, labels)

    # Convert pairs to tensors
    pair1 = torch.stack([pair for pair in pairs])
    pair2 = torch.stack([pair for pair in pairs])

    # Compute loss
    loss = contrastive_loss(pair1, pair2, pair_labels)
    return loss
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                for i in range(args.num_steps):
                    ...
                    inner_loss = compute_contrastive_loss(train_embeddings, train_target)#+simple_triplet_loss(train_embeddings, train_target)
