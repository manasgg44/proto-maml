...
def test(args):
    ...
    model = PrototypicalNetwork_new(1,args.embedding_size,hidden_size=args.hidden_size)                                
    #                               
    #Save model
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            #with torch.no_grad():
                ...
                for task_idx, (train_input, train_target, test_input,
                    ...
                    params=None
                    train_embeddings_original = model.forward_enc(train_input)

                    for i in range(args.num_steps):
                        train_embeddings = model.forward_enc(train_input,params=params)
                        tl = simple_triplet_loss(train_embeddings, train_target)
                        cl = simple_contrastive_loss(train_embeddings, train_target)
                        inner_loss = cl+tl
