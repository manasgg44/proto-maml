...
def train(args):
    ...
    model = PrototypicalNetwork_new(3,
                                       args.embedding_size,
    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                params=None
                #model.zero_grad()
                #train_input_supp = train_input[]
                #print(train_target)
                train_input_query = train_input[args.num_shots-1::args.num_shots]
                train_input_supp = torch.tensor([tensor.detach() for idx, tensor in enumerate(train_input) if (idx + 1) % 5 != 0])
                train_target_query = train_target[args.num_shots-1::args.num_shots]
                train_target_supp = torch.tensor([tensor.detach() for idx, tensor in enumerate(train_target) if (idx + 1) % 5 != 0])
                print(train_target)
                print(train_target_query)
                print(train_target_support)
                #train_embeddings_original = model.forward_enc(train_input, params=params)
                for i in range(args.num_steps):
                    train_embeddings = model.forward_enc(train_input_supp, params=params)
