    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                train_embedding = model(train_input.unsqueeze(0))
                test_embedding = model(test_input.unsqueeze(0))

                prototypes = get_prototypes(train_embedding, train_target.unsqueeze(0),
                dataset.num_classes_per_task)
                loss += prototypical_loss(prototypes, test_embedding, test_target.unsqueeze(0))

                with torch.no_grad():
                    acc = get_accuracy(prototypes, test_embedding, test_target.unsqueeze(0))
                    accuracy += acc
                    #print(acc)
