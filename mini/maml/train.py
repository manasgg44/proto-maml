    ...
    with tqdm(dataloader, total=args.num_batches) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                train_embeddings_original = model.forward_enc(train_input.unsqueeze(0), params=params)
                for i in range(args.num_steps):
                    train_embeddings = model.forward_enc(train_input.unsqueeze(0),params=params)
                    inner_loss = simple_contrastive_loss(train_embeddings.squeeze(), train_target)+simple_triplet_loss(train_embeddings.squeeze(), train_target)
                    #print("Hi")
                ...
                outer_loss += prototypical_loss(prototypes,test_embeddings, test_target.unsqueeze(0))
		#loss.backward()
                #optimizer.step()
                with torch.no_grad(): 
...
if __name__ == '__main__':
    ...
    args.device = torch.device('cuda:3' if args.use_cuda
